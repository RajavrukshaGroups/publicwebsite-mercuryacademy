...
//     } else if (response) {
//       // Handle the case where the response is an object with numerical keys
...
export default async function UniversitiesPage() {
  ...
  try {
    const response = await getUniversities();
    const baseUniversities = Array.isArray(response.data) ? response.data : [];

    // Fetch full details for each university to get the populated logo
    universities = await Promise.all(
      baseUniversities.map(async (university) => {
        try {
          if (university.slug) {
            const detailResponse = await getUniversityBySlug(university.slug);
            if (detailResponse?.data?.logo) {
              return { ...university, logo: detailResponse.data.logo };
            }
          }
        } catch (err) {
          console.error(`Error fetching details for university ${university.slug}:`, err);
        }
        return university;
      })
    );
